...
export class ScoringsComponent implements OnInit {
  ...
  constructor(private _soccerService: SoccerService) { 
    ...
    this.AwayTeam = "";
    this.getSchedule();
    this.SeasonStart.setTime(this.SeasonStart.getTime()+4 * 8640000);
  ...
  public onRecordScores(){
    this.MySchedule[this.CurGame-1].AwayScore = Number(this.AwayTeam);
    this.MySchedule[this.CurGame-1].HomeScore = Number(this.HomeTeam);
  }
